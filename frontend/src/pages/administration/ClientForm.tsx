...
export default function ClientForm() {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormValues) => {
      return (await api.post('/admin/clients/', { ...data, email: data.email || null })).data
    },
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Registre un nuevo cliente</p>
        </div>
      </div>
          ...
          <CardBody className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <FormField label="Nombre / Razón Social" required error={errors.name?.message}>
                <input {...register('name')} className={inputClass} placeholder="Nombre del cliente" />
              </FormField>
            </div>
